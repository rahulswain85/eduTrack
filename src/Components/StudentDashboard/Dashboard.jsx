  ...
  return (
    <>
      <div className="flex flex-col gap-2 flex-wrap justify-center items-center h-full">
        ...
        <div className="flex-5 bg-indigo-50 w-full">
          <div className="flex md:flex-row gap-3 flex-col flex-wrap w-full p-4 justify-center items-center">
            <div className="flex flex-col flex-wrap bg-indigo-400 md:w-[20%] shadow inset-shadow-sm/20 w-full min-h-[20vh] p-4 rounded-2xl">
              <p className="md:text-4xl lg:text-3xl text-xl font-bold">0</p>
              <p>Total Tasks</p>
            </div>
        ...
        <div className="bg-white flex flex-wrap flex-row gap-3 w-full min-h-[calc(100vh-50vh)] p-6 max-h-[calc(100vh-50vh)] rounded-2xl">
          {currentStudentTasks().map((task) => (
            <div
              key={task.taskId}
              className="md:w-[25%] w-full p-4 h-full flex flex-col border-l-5 inset-shadow-sm/30 border-indigo-400 gap-2 rounded-2xl shadow hover:shadow-md justify-baseline items-start"
            >
              <p
              ...
              >
                {task.taskPriority}
                <AiOutlineRise
                  className={`${task.taskPriority === "High" ? `text-red-500` : task.taskPriority === "Medium" ? `text-amber-400` : `text-green-500`}`}
                />
              </p>
              <hr className="w-full " />
              <p className="text-2xl font-bold text-indigo-500">{task.taskTitle}</p>
              <p className="italic font-semibold">{task.taskDueDate}</p>
              <select onChange={(e)=>handleStatusChange(task.taskId, e.target.value)} value={task.taskStatus} className={`self-center w-full p-1 mt-2 rounded-2xl inset-shadow-sm/20 font-semibold ${task.taskStatus === 'Pending' ? `bg-red-100` : task.taskStatus === 'In Progress' ? `bg-amber-100` : `bg-green-100`}`}>
